const accepted_choices = ['ROCK', 'PAPER', 'SCISSORS'];
...
function getComputerChoice() {
    return accepted_choices[
        (Math.floor(Math.random() * accepted_choices.length))
    ];
}
...
function getPlayerChoice() {
    ...
    if (playerSelection === null || playerSelection.trim() === "" || !accepted_choices.includes(playerSelection.toUpperCase())) {
        console.error(`Allowed options: ${accepted_choices.join(", ")}. Try again!`);
        return null;
    }
...
function announceWinner(playerScore, computerScore) {
    ...
    return (playerScore > computerScore) ? "You won the challenge" : "You lost the challenge";
}
